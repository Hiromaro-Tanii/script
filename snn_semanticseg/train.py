...
class LoadDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, i):
        ...
        image = image.reshape(4096,10)##64pix × 64pix, 11step?
...
parser.add_argument('--epoch', '-e', type=int, default=10)##エポック数を指定。指定しない場合100になる。
...
print("***************************")##出力
train_dataset = LoadDataset("snn_semanticseg/data/csv_data/semantic_train_loc.csv")##教師用データ
test_dataset = LoadDataset("snn_semanticseg/data/csv_data/semantic_eval_loc.csv")##評価用データ
data_id = 2
...
for epoch in tqdm(range(epochs)):
    ...
    # モデル保存
    if epoch == 0 :
        torch.save(model.state_dict(), "snn_semanticseg/models/models_state_dict_"+str(epoch)+"epochs.pth")
